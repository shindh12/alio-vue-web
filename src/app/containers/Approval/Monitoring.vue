    ...
    <div class="isdp-body-content">
        ...
        <div class="page-searchbox page-searchbox--col3">
            ...
            <v-select :items="systemList" v-model="searchCondition.systemKey" item-value="systemKey" item-text="systemName" />
                ...
                <div class="page-searchbox__item-cell">
                    <div class="page-searchbox__item-title">{{message['approval-type']}}</div>
                    <span class="---has-to-be-span---">
            <v-select :items="approvalTypeList" v-model="searchCondition.approvalType" item-value="value" item-text="text" class="narrow-search-select"></v-select>
          </span>
                </div>
            </div>
            ...
            <div class="page-searchbox__row">
                ...
                <div class="page-searchbox__item-cell">
                    <v-text-field name="searchSubmissionUser"
                                  :placeholder="'\''+message['username'] + '\' ' + this.message['or'] + ' \'' + message['id'] + '\''"
                                  :label="message['submission-user']"
                                  v-model.trim="searchCondition.submissionUserText"
                                  @keyup.enter="callbackPagination(1)"
                                  maxlength="30" />
                </div>
                <div class="page-searchbox__item-cell">
                    <v-text-field name="searchApprovalUser"
                                  :placeholder="'\''+message['username'] + '\' ' + this.message['or'] + ' \'' + message['id'] + '\''"
                                  :label="message['current-approval-user']"
                                  v-model.trim="searchCondition.approvalUserText"
                                  @keyup.enter="callbackPagination(1)"
                                  maxlength="30" />
                </div>
            </div>
            ...
            <div class="page-searchbox__row">
                <div class="page-searchbox__item-cell page-searchbox__item-cell--colspan2">
                    ...
                    <div class="page-searchbox__item__select-wrapper" v-for="status in statusCodeList" :key="status.value">
                        <v-checkbox v-model="searchCondition.multipleStatusCode" :label="status.text" :value="status.value"></v-checkbox>
                    </div>
                </div>
            </div>
                    ...
                    >
                        ...
                        <template slot="items" slot-scope="props">
                            <tr>
                                ...
                                <!-- 결재 유형 -->
                                <td class="text-xs-center">{{ getApprovalTypeName(props.item.approvalType) }}</td>
                                <!-- 제목 -->
        ...
        <!-- Dialog Area -->
        <monitoring-detail-dialog :showDialog="detailDialog" @closeDialog="closeDetailDialog" :approval="selectedApproval"
                                  :files="selectedApprovalFiles"></monitoring-detail-dialog>
    </div>
</template>
...
<script>
    ...
    import MonitoringDetailDialog from './ApprovalDialog/MonitoringDetailDialog';
    ...
    export default {
        ...
        data() {
            return {
                ...
                searchCondition: {
                    ...
                    approvalName: '',
                    approvalType: '',
                    multipleStatusCode: ['INPROGRESS', 'COMPLETE', 'DISAPPROVAL', 'CANCEL'],
                    submissionUserLogin: '',
                    submissionUserId: '',
                    submissionUserText: '',
                    approvalUserLogin: '',
                ...
                message: {
                    ...
                    , 'approval-type': ''// 결재 유형
                    , 'pending-user': ''// 미결자
                    , 'internal-approval': ''// 내부결재
                    , 'external-approval': ''// 외부결재
                    , 'system': ''// 시스템
                    , 'search': ''
                    , 'date-range-error': ''
        ...
        methods: {
            movePage(index) {
                ...
                axios.get('/rest/v1/approval/monitoring'
                    , {
                        params: {
                            systemKey: this.searchCondition.systemKey,
                            approvalType: this.searchCondition.approvalType,
                            approvalName: this.searchCondition.approvalName,
                            multipleStatusCode: this.searchCondition.multipleStatusCode.join(),
                            submissionUserLogin: this.searchCondition.submissionUserLogin,
                            submissionUserId: this.searchCondition.submissionUserId,
                            submissionUserText: this.searchCondition.submissionUserText,
                            approvalUserLogin: this.searchCondition.approvalUserLogin,
                            approvalUserName: this.searchCondition.approvalUserName,
                            approvalUserText: this.searchCondition.approvalUserText,
                            fromSubmissionDateText: this.searchCondition.fromSubmissionDateText,
                            toSubmissionDateText: this.searchCondition.toSubmissionDateText,
                            userId: this.$store._modules.root.state.loginStore.currentUser.userId,
                            page: index - 1,
            ...
            getApprovalTypeName(type) {
                ...
                let filteredApprovalType = this.approvalTypeList.filter((ApprovalType) => {
                    if (ApprovalType.value == type) {
                        return ApprovalType;
                    }
                });
            ...
            messageLoad() {
                ...
                this.message['approval-type'] = this.$i18n.td('approval.approval-type', '결재유형');
                this.message['pending-user'] = this.$i18n.td('approval.pending-user', '미결자');
                this.message['internal-approval'] = this.$i18n.td('approval.internal-approval', '내부결재');
                this.message['external-approval'] = this.$i18n.td('approval.external-approval', '외부결재');
            ...
            getGridHeader() {
                let newGridHeader = [
                    ...
                    {text: this.message['approval-type'], align: 'center', sortable: false, value: 'approvalType', width: '80px'},
                    ...
                    {text: this.message['current-approval-user'], align: 'center', sortable: false, value: 'currentApprovalUserName', width: '150px'},
            ...
            getApprovalTypeList() {
                ...
                let newApprovalTypeList = [
                    ...
                    , {text: this.message['internal-approval'], value: 'INTERNAL'}
                    , {text: this.message['external-approval'], value: 'EXTERNAL'}
                ];
                this.approvalTypeList = newApprovalTypeList;
