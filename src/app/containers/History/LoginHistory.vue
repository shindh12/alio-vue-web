        ...
        <div class="page-searchbox page-searchbox--col2">
            ...
            <div class="page-searchbox__row">
                <div class="page-searchbox__item-cell">
                    <v-text-field name="searchText" :label="$td('message.user', '사용자')" v-model.trim="searchText" @keyup.enter="callbackPagination(1)" maxlength="30"
                ...
                <div class="table__body">
                    <v-data-table :headers="headers" :items="items" item-key="id"
                        ...
                        <template slot="items" slot-scope="props">
                            ...
                            <td class="text-xs-center" v-if="props.item.loginStatus=='SUCCESS'">{{ $td('message.login-success', '로그인 성공') }}</td>
                            <td class="text-xs-center" v-if="props.item.loginStatus=='FAIL'">{{ $td('message.login-fail', '로그인 실패') }}</td>
                            <td class="text-xs-center" v-if="props.item.loginStatus=='LOGOUT'">{{ $td('message.logout', '로그아웃') }}</td>
    ...
    export default {
        ...
        data() {
            return {
                ...
                headers: [
                    ...
                    {text: this.$i18n.td('user.ip', '사용자 IP'), align: 'center', sortable: false, value: 'ip', width: '180px'},
                    {text: this.$i18n.td('message.result', '결과'), align: 'center', sortable: false, value: 'loginStatus', width: '100px'},
        ...
        methods: {
            ...
            searchLoginHistoryList() {
                const url = '/rest/v1/history/login/list';
            ...
            setDownloadHeaders() {
                ...
                this.downloadHeaders[this.$i18n.td('user.ip', '사용자 IP')] = 'ip';
                this.downloadHeaders[this.$i18n.td('message.result', '결과')] = 'loginStatus';
            ...
            async getExcelList() {
                const url = '/rest/v1/history/login/list/filtered';
                ...
                for (let i in this.excelData.data) {
                    ...
                    if (this.excelData.data[i].loginStatus == 'SUCCESS') {
                        row.loginStatus = this.$i18n.td('message.login-success', '로그인 성공');
                    } else if (this.excelData.data[i].loginStatus == 'FAIL') {
                        row.loginStatus = this.$i18n.td('message.login-fail', '로그인 실패');
                    } else if (this.excelData.data[i].loginStatus == 'LOGOUT') {
                        row.loginStatus = this.$i18n.td('system.logout', '로그아웃');
                    }
...
<style>
    #login-history-page table.datatable {
        table-layout: fixed
    }
</style>
