        ...
        <v-dialog :value="userAddEditDialog" @input="closeDialog" max-width="400px">
            <v-card id="user-add-edit-popup">
                ...
                <v-card-text class="field-box">
                    <tb-validator class="tb-validator-in-dialog" form v-model="validUser" ref="form-valid" @click.native="clearValid">
                        <tb-validator :target="editedItem.login" :rules="[rules.required, rules.login, rules.nospace]" lazy>
                            <v-text-field ref="login" :label="$i18n.td('user.id', '아이디')" v-model.trim="editedItem.login" :disabled=isDisabled required maxlength="12" />
                        </tb-validator>
                        <tb-validator :target="editedItem.fullName" :rules="[rules.required, rules.fullName, rules.nospace]" lazy>
                        ...
                        <tb-validator :target="editedItem.email" :rules="[rules.required, rules.email]" lazy>
