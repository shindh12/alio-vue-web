<template>
    <div class="approval-dialog">
        ...
        <v-dialog :value="isInternalMailWriteDialog" fullscreen hide-overlay @input="checkCurrentMenuDirty" transition="dialog-bottom-transition">
...
<script>
    ...
    export default {
        components: {vueDropzone, ExternalUserSearchDialog, InternalUserSearchDialog, CommonConfirm},
        ...
        methods: {
            ...
            searchInternalUser(key) {
                ...
                axios
                .post(CONTEXT_USER, {searchText: key, systemKey: this.$store._modules.root.state.loginStore.systemKey})
                .then(response => {
            ...
            fileAddedEvent(file) {
                ...
                /* Duplicate with "this.mailItem.newFileList"  this file list is located in first box  */
                if (this.mailItem.newFileList.length > 0) {
                    for (let i in this.mailItem.newFileList) {
                        if (this.mailItem.newFileList[i].fileName === file.name) {
                            this.$refs.myVueDropzone.removeFile(file);
                            this.$notify({title: this.message['attach-file'], text: `${this.message['already-attached']} ( ${file.name} )`, type: 'warn'});
                        }
                    }
                }
                /* Duplicate with new attachment    this file list is located in dropzone */
            ...
            messageLoad() {
                ...
                this.message['menu-mail.add-recipient'] = this.$i18n.td('menu-mail.add-recipient', '수신인 추가');
            ...
            checkCurrentMenuDirty() {
                if (this.mailItem.title !== '' || this.mailItem.recipient.length > 0 || this.mailItem.content !== '') {
