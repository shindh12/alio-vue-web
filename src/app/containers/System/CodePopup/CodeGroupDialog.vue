            ...
            <v-card>
                ...
                <v-card-text class="field-box">
                    <tb-validator class="tb-validator-in-dialog" form v-model="createValid" ref="form-valid" @click.native="clearValid">
                        <tb-validator :target="editedItem.groupKey" :rules="[rules.required, rules.groupDup, rules.codeKey,rules.nospace]" lazy>
                            <v-text-field
                                name="groupKey"
                                ref="groupKey"
                                :placeholder="$td('code.group-key', '그룹키')"
                                :label="$td('code.group-key', '그룹키')"
                                v-model.trim="editedItem.groupKey"
                                @keyup="checkGroupDuplicate"
                                @change="changeUpdateMode"
                ...
                <v-card-actions>
                    ...
                    <v-btn primary v-if="saveBtnAuth" @click.native="saveCodeGroup()">{{ $td('button.save', '저장') }}</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        <!--</v-form>-->
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            title() {
                return this.mode == 'ADD' ? this.$i18n.td('menu.code-group-create', '코드 그룹 생성') : this.$i18n.td('menu.code-group-update', '코드 그룹 수정');
            },
        },
        watch: {
            showDialog(status) {
        ...
        methods: {
            ...
            saveCodeGroup() {
                ...
                let codeDetail = {
                    ...
                    'groupKey': this.editedItem.groupKey,
                ...
                if (this.mode === 'ADD') {
                    ...
                    codeDetail.groupId = this.editedItem.groupId;
                    codeDetail.useYn = this.editedItem.useYn;
                    axios.post('/rest/v1/code/' + this.editedItem.groupId + '/detail', codeDetail)
                    .then(response => {
                        this.$notify({title: this.$i18n.td('button.save', '저장'), text: this.$i18n.td('message.saved', '저장되었습니다.'), type: 'success'});
                        this.$emit('reloadData');
                        this.closeDialog();
            ...
            checkGroupDuplicate: _.debounce(
                function () {
                    ...
                    if (!this.editedItem.groupKey) {
                        this.groupDuplicate = false;
                        return false;
                    }
                    if (!this.mode === 'ADD' && this.editedItem.beforeGroupKey == this.editedItem.groupKey) {
                        this.groupDuplicate = false;
                    } else {
                        axios.get('/rest/v1/code/groupkey/' + this.editedItem.groupKey)
                        .then(response => {
