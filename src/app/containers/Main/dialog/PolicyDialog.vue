<template>
    <div>
        <v-dialog :value="showDialog" @input="closeDialog" max-width="500px">
            <v-card id="policy-dialog-popup">
                <v-card-title>
                    <span class="headline"> {{ td.menu.policy }} </span>
                </v-card-title>
                ...
                <v-card-text class="field-box">
                    <div>
                        <table>
                            <tr>
                                <td>{{ policyInform.description }}</td>
                            </tr>
                            <tr>
                                <td>{{ policyInform.information }}</td>
                            </tr>
                        </table>
                    </div>
                </v-card-text>
    ...
    export default {
        ...
        data() {
            return {
                ...
                td: {
                    menu: {
                        policy: this.$i18n.td('menu.privacy', '개인정보처리방침'),
                    },
                    message: {
