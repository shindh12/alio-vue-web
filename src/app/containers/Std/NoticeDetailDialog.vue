<template>
    <div class="isdp-body-content" id="approval-detail-dialog">
        <v-dialog :value="showDialog" @input="closeDialog" fullscreen hide-overlay
                  transition="dialog-bottom-transition">
            <v-card class="approval-writing" id="approval-detail-dialog"><!-- writing??? -->
                <!-- Title Area -->
                <v-toolbar>
                    <v-toolbar-title>
                        <span class="headline">공지사항 상세조회</span>
                    </v-toolbar-title>
                    <v-spacer></v-spacer>
                ...
                <v-card-text class="dialog-body aprv__content">
                    ...
                    <div class="aprv__content__row detail-page__contents">
                        <table class="detail-page__contents__table">
                            ...
                            <tr>
                                <th>작성자</th>
                                <td>{{ notice.fullName + '(' + notice.login + ')' }}</td>
                            </tr>
                            <tr>
                                <th>작성일시</th>
                                <td>{{ notice.updateDate | moment(dateTimePattern()) }}</td>
                            </tr>
                            <tr>
                                <th>사용여부</th>
                                <td>{{ notice.useYn =='Y' ? $td('message.use-y', '사용') : $td('message.use-n', '미사용')}}</td>
                            </tr>
                        </table>
                    </div>
                    ...
                    <div class="aprv__content__row aprv__content__html" style="">
                        <p v-html="notice.noticeContents"></p>
                    </div>
                    <!-- [CONTENT] 결재본문 END-->
                </v-card-text>
                ...
                <v-card-actions>
                    <div class="action-row--mini ">
                        ...
                        <div class="right">
                            <v-btn @click.native="openConfirmDialog" v-if="menuAuth.delete">{{$td('button.delete',
                                '삭제')}}
                            </v-btn>
                            <v-btn primary @click.native="openUpdateDialog(notice)" v-if="menuAuth.update">
                                {{$td('button.update', '수정')}}
                            </v-btn>
                        </div>
                    </div>
